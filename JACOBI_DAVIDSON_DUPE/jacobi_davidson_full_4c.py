...
class JacobiDavidsonFull4C(eps_solvers.Solver):
    ...
    def check_and_save_rh(self):
        ...
        for ii in range(self.vspace_r.shape[1]):
            utils.save_arrs_to_file([self.vspace_r[:, ii], self.vspace_rp[:, ii], self.wspace_r[:, ii],
                                     self.wspace_rp[:, ii]],
                                    ["v_vec_r_c" + str(self.cycle) + "_i" + str(ii + 1),
                                     "v_vec_rp_c" + str(self.cycle) + "_i" + str(ii + 1),
                                     "w_vec_r_c" + str(self.cycle) + "_i" + str(ii + 1),
                                     "w_vec_rp_c" + str(self.cycle) + "_i" + str(ii + 1)])
    ...
    def save_spaces(self):
        if self.vspace_r is not None:
            self.save_array_as_vectors(self.vspace_r, "v_vec_r_c"+str(self.cycle))
        if self.vspace_rp is not None:
            self.save_array_as_vectors(self.vspace_rp, "v_vec_rp_c"+str(self.cycle))
        if self.vspace_l is not None:
            self.save_array_as_vectors(self.vspace_l, "v_vec_l_c"+str(self.cycle))
        if self.vspace_lp is not None:
            self.save_array_as_vectors(self.vspace_lp, "v_vec_lp_c"+str(self.cycle))
        if self.wspace_r is not None:
            self.save_array_as_vectors(self.wspace_r, "w_vec_r_c"+str(self.cycle))
        if self.wspace_rp is not None:
            self.save_array_as_vectors(self.wspace_rp, "w_vec_rp_c"+str(self.cycle))
        if self.wspace_l is not None:
            self.save_array_as_vectors(self.wspace_l, "w_vec_l_c"+str(self.cycle))
        if self.wspace_lp is not None:
            self.save_array_as_vectors(self.wspace_lp, "w_vec_lp_c"+str(self.cycle))
