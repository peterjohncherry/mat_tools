...
def test_fortran_file_read():
    nrows, ncols, datatype = mr.read_array_info_file('REF/MAT_READER/mat1_test.info')
    print("nrows = ", nrows, "  ncols = ", ncols)
    mr.read_binary_fortran_file('REF/MAT_READER/mat1_test.bin', datatype, nrows, ncols)


def test_jacobi_davidson_tda_4c():
    nevals = 3
    jd_test = jdr4c.JacobiDavidsonTDA4C(num_eigenvalues=nevals,
                                        rs_filename="REF/4c-HF.out_scf")
    jd_test.read_full_matrix(file_seedname="REF/TDA/full_mat")
    jd_test.solve()
...
def test_jacobi_davidson_full_4c():
    ...
    jd_test = jd_f_4c.JacobiDavidsonFull4C(num_eigenvalues=nevals,
                                           rs_filename="REF/4c-HF.out_scf")
    jd_test.read_full_matrix(file_seedname="REF/FULL/full_mat")
    jd_test.solve()


def test_array_reading():
    evals = mr.read_fortran_array("REF/lapack_eigvals")
    evals = np.float64(evals)
    np.savetxt("REF/lapack_eigvals", evals, fmt='%10.5f')
